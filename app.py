...
if not st.session_state.submitted:
    ...
    with st.form("survey_form"):
        ...
        responses['module1'][1] = st.radio(
            f"**{question_num}.** 学校老师反馈孩子上课专注力情况",
            ["A", "B", "C", "D"] if not DEFAULT else ["A", "B", "C", "D"],
        ...
        responses['module1'][2] = st.radio(
            f"**{question_num}.** 你在家庭观察孩子写作业时，是否频繁中途停下（吃零食、玩橡皮、刷手机），正常时长能完成的作业常拖延1倍以上时间？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][3] = st.radio(
            f"**{question_num}.** 孩子做手工、搭积木时，是否经常中途放弃（做一半换其他事），无法专注解决小问题（如积木搭不稳）？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][4] = st.radio(
            f"**{question_num}.** 孩子做题（如数学计算、语文填空）时，是否经常出现粗心马虎（如抄错数字、看错题号、漏填答案）？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][5] = st.radio(
            f"**{question_num}.** 孩子绘画、连线时，是否经常出线、涂错区域，无法让线条沿轮廓顺畅移动（如画圆成椭圆）？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][6] = st.radio(
            f"**{question_num}.** 孩子抄写、拼拼图时，是否频繁看错行、找不到位置，耗时远超同龄人？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][7] = st.radio(
            f"**{question_num}.** 关于握笔姿势及力度，孩子通常更符合以下哪种情况？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][8] = st.radio(
            f"**{question_num}.** 当需要完成书写类作业（笔头作业）时，孩子的状态更接近？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][9] = st.radio(
            f"**{question_num}.** 孩子完成书写任务（如写10分钟作业、抄一段话）后，是否频繁抱怨手腕酸手指累，甚至主动停下不愿继续？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][10] = st.radio(
            f"**{question_num}.** 阅读准确性与理解：孩子阅读时，是否经常漏字、添字、颠倒错读（如朋友读友朋）？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][11] = st.radio(
            f"**{question_num}.** 书写准确性：孩子抄写/默写时，是否频繁笔画颠倒（如b写d）、形近字混淆（如人写入），纠正后仍错？",
            ["A", "B", "C", "D"],
        ...
        st.subheader("理解力、记忆力")
        
        responses['module1'][13] = st.radio(
            f"**{question_num}.** 孩子学新知识点时，能否主动联系之前学过的内容（如这个和上次学的XX很像），说出它们的关联？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][14] = st.radio(
            f"**{question_num}.** 孩子学过的诗词、公式、单词等内容，间隔1-2周后，能否不提示就准确背出来或写出来？",
            ["A", "B", "C", "D"],
        ...
        responses['module1'][15] = st.radio(
            f"**{question_num}.** 面对与学过知识点相关的提问或考试，孩子能否回忆起关键内容并正确答出？",
            ["A", "B", "C", "D"],
        ...
        st.write(f"**{question_num}.** 孩子当时的出生情况（可多选）")
        ...
        responses['module1'][17] = st.radio(
            f"**{question_num}.** 孩子6岁前，家人对孩子喂饭、整理餐具等事的包办程度？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][1] = st.radio(
            f"**{question_num}.** 孩子遇到困难时，向家庭成员求助后，能否得到耐心的帮助和鼓励？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][2] = st.radio(
            f"**{question_num}.** 您和孩子是否经常因学习问题（如作业拖延、成绩波动）发生冲突（如争吵、孩子抵触）？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][3] = st.radio(
            f"**{question_num}.** 除了学习以外，您和孩子是否经常出现情绪对抗（如孩子反驳、冷战，您批评、急躁）？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][4] = st.radio(
            f"**{question_num}.** 孩子在学校遇到学习或社交困难时，是否愿意主动去找老师沟通？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][5] = st.radio(
            f"**{question_num}.** 孩子平时如何评价和描述他/她的老师的？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][6] = st.radio(
            f"**{question_num}.** 根据您的观察，孩子与老师沟通后，问题通常能得到怎样的解决？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][7] = st.radio(
            f"**{question_num}.** 孩子经常交往的同学或朋友，是否大多对学习抱有积极态度（按时完成作业、愿意探索新知识）？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][8] = st.radio(
            f"**{question_num}.** 孩子和同伴在一起时，是否会交流学习内容（如讨论题目、分享方法），或在学习上互相鼓励、良性竞争？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][9] = st.radio(
            f"**{question_num}.** 孩子在学校是否有固定一起玩的好朋友（比如下课、午休、放学一起活动）？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][10] = st.radio(
            f"**{question_num}.** 孩子能否看到自己的优点（如我画画好），即使有不足，也能接纳自己（如我写字慢，但我写得工整）？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][11] = st.radio(
            f"**{question_num}.** 当遇到难题或考试失利时，孩子通常如何反应？",
            ["A", "B", "C", "D"],
        ...
        responses['module2'][12] = st.radio(
            f"**{question_num}.** 一次失败（如比赛输了、作业错很多）是否会让孩子轻易放弃对这件事的兴趣（如不再参加比赛、不愿写作业）？",
            ["A", "B", "C", "D"],
        ...
        responses['module3'][1] = st.radio(
            f"**{question_num}.** 当学习一首古诗时，孩子更倾向于：",
            ["A", "B", "C"],
        ...
        responses['module3'][2] = st.radio(
            f"**{question_num}.** 记忆三角形这个英文单词时，孩子觉得最容易的方法是：",
            ["A", "B", "C"],
        ...
        responses['module3'][3] = st.radio(
            f"**{question_num}.** 在理解守株待兔这个成语时，什么方式对孩子帮助最大？",
            ["A", "B", "C"],
        ...
        responses['module3'][4] = st.radio(
            f"**{question_num}.** 复习功课时，孩子最常用的方法是：",
            ["A", "B", "C"],
        ...
        responses['module3'][8] = st.radio(
            f"**{question_num}.** 当需要记住一个历史事件时，孩子更喜欢：",
            ["A", "B", "C"],
        ...
        responses['module3'][9] = st.radio(
            f"**{question_num}.** 学习对称这个概念时，什么活动让孩子理解最深刻？",
            ["A", "B", "C"],
        ...
        responses['module3'][10] = st.radio(
            f"**{question_num}.** 孩子完成家庭作业（如语文组词、科学观察）时，会主动选择哪种辅助方式？",
            ["A", "B", "C"],
        ...
        responses['module3'][11] = st.radio(
            f"**{question_num}.** 组装一个模型玩具时，孩子会怎么做？",
            ["A", "B", "C"],
        ...
        responses['module3'][12] = st.radio(
            f"**{question_num}.** 孩子向你分享学校趣事时，更倾向于哪种表达？",
            ["A", "B", "C"],
            ...
            if not DEFAULT:
                for i in range(1, 16):
                    if responses['module1'].get(i) is None:
                        errors.append(f"请回答第{i}题")
                
                if not responses['module1'][16]:
                    errors.append("请至少选择一个选项（第16题）")
                
                if responses['module1'].get(17) is None:
                    errors.append("请回答第17题")
                if responses['module1'].get(18) is None:
                    errors.append("请回答第18题")
                
                for i in range(1, 13):
                    if responses['module2'].get(i) is None:
                        errors.append(f"请回答第{18+i}题")
                
                for i in range(1, 13):
                    if responses['module3'].get(i) is None:
                        errors.append(f"请回答第{30+i}题")
            else:
                if not responses['module1'][16]:
                    errors.append("请至少选择一个选项（第16题）")
